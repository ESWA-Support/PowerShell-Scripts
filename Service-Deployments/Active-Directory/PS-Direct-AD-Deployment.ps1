...
#>
# Command neded to get make all of this work 

 
foreach ($I in $VSwtich){

        Write-Output $VSwtich.Id
}


$VSwtich = Get-VMSwitch
$menu = @{}
for ($i=1;$i -le $VSwtich.count; $i++) 
{ Write-Host "$i. $($VSwtich[$i-1].name)"
$menu.Add($i,($VSwtich[$i-1].name)) }

[int]$ans = Read-Host 'Enter selection'
$selection = $menu.Item($ans) ; Get-VMSwitch $selection | Select-Object -Property *
